---
name: sc-cli-portify
description: "Port inference-based SuperClaude workflows (commands + skills + agents) into programmatic CLI pipelines with sprint-style supervised execution. Use this skill when you need to convert any multi-step workflow into a reliable, repeatable CLI runner that uses Python to control flow and Claude to fill in content. Triggers on: portify, CLI pipeline, programmatic workflow, convert skill to CLI, workflow runner, deterministic pipeline, move to code, make repeatable, ensure consistency."
