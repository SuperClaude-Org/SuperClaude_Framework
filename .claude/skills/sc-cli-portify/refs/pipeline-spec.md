...
EXEMPT_GATE = GateCriteria(tier="exempt")

# Light: file exists and non-empty
LIGHT_GATE = GateCriteria(tier="light")
...
STRICT_GATE = GateCriteria(
    tier="strict",
    min_lines=100,
    required_frontmatter=["title", "status", "convergence_score"],
    semantic_checks=[
        SemanticCheck(name="no_heading_gaps", fn=_no_heading_gaps),
        SemanticCheck(name="refs_resolve", fn=_cross_refs_resolve),
    ],
)
```
...
Semantic checks are pure functions that take file content and return `(passed: bool, reason: str)`:

```python
def _no_heading_gaps(content: str) -> tuple[bool, str]:
    """Verify heading hierarchy has no gaps (H1->H3 without H2)."""
    # ... implementation ...
    return (True, "") if valid else (False, f"Heading gap at line {line}")

def _has_required_sections(content: str) -> tuple[bool, str]:
