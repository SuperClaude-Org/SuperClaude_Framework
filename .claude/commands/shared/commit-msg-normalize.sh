...
#
# Enforces:
#   - Valid type prefix: feat|fix|docs|style|refactor|test|chore|perf|ci
#   - Lowercase type
...
#
# Skip: set SKIP_COMMIT_MSG_HOOK=1 or use git commit --no-verify
...
# Valid conventional commit types (full Conventional Commits spec)
readonly VALID_TYPES="feat|fix|docs|style|refactor|test|chore|perf|ci"
...
# Pass through merge commits (generated by git merge)
if printf '%s' "$raw_msg" | head -n1 | grep -qE '^Merge (branch|remote-tracking|tag) '; then
  exit 0
fi

# Pass through fixup and squash commits (generated by git rebase -i)
if printf '%s' "$raw_msg" | head -n1 | grep -qE '^(fixup|squash)! '; then
  exit 0
fi

# Pass through amend commits with no message change
if printf '%s' "$raw_msg" | head -n1 | grep -qE '^amend! '; then
  exit 0
fi
...
if ! printf '%s' "$subject" | grep -qE "^[A-Za-z]+(\([A-Za-z0-9._-]+\))?\!?:"; then
  ...
  printf "    perf(db): optimize query execution\n" >&2
  printf "    ci: update GitHub Actions workflow\n" >&2
  exit 1
fi
