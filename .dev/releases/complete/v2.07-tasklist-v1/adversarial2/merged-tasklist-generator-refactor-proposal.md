...
<!-- Base: Variant A (tasklist-generator-v3-refactor-proposal.md) -->
...
The current Tasklist Generator Prompt (v2.1-unified / v2.2) produces a **single monolithic markdown document**. The Sprint CLI (`superclaude sprint run`) requires a **tasklist-index.md + per-phase `phase-N-tasklist.md` files**. This is the fundamental mismatch. The prior refactor notes correctly identified 10 required changes. This document deepens that analysis with code-level evidence, concrete template specifications, and a prioritized refactoring plan.

---
<!-- Source: Base (original, modified) — conciseness pass per Change #4 -->

## Part 1: Sprint CLI Contract (Code-Evidenced)

### 1.1 Phase File Discovery -- The Hard Constraint

Phase files are discovered via regex in `config.py:17-25`:
...
**Generator requirement**: Emit files named `phase-N-tasklist.md` (canonical convention). The index must contain literal filename references that match the regex.
...
Each phase file must begin with `# Phase N — <Name>` (or `: ` separator). Name portion must not exceed 50 characters.
...
- Phase files are passed whole as `@file` context -- they must be **lean and execution-focused**
- Heavy metadata (Deliverable Registry, Traceability Matrix, templates) bloats the prompt unnecessarily
- The completion protocol is injected by the executor, not the phase file -- phase files must NOT include completion instructions
- Each phase file is an independent, self-contained execution unit
...
Task IDs must be `T<PP>.<TT>` with zero-padded 2-digit phase and task numbers.
...
The command (`task-unified.md`) performs text-based tier classification before execution. When a phase file is passed, the entire session is classified at the highest-priority tier present:
...
**Required**: Standalone `tasklist-index.md` with literal `phase-N-tasklist.md` references matching the regex pattern
...
**Required**: Phase files contain ONLY their phase tasks + phase-level context
**Impact**: Each phase session receives unnecessary tokens

### GAP-04: Completion Protocol Redundancy (MEDIUM)

**Current**: Generator embeds Execution Log Template, Checkpoint Report Template
**Required**: Sprint executor injects its own Completion Protocol; generator templates are redundant
...
**Required**: Each phase file should have an end-of-phase checkpoint as the gate for the next phase
...
|---|---|
| Section 1 Objective | Remove "Single-document"; add "Multi-file bundle" |
| Section 3 Artifact Paths | Add `tasklist-index.md` and `phase-N-tasklist.md` to standard paths |
| Section 4.8 Checkpoints | Keep inline checkpoints; add mandatory end-of-phase checkpoint |
| Section 6 Output Template | Replace monolithic template with Index Template + Phase File Template |
| Section 6.1-6.4 | Move to Index File Template |
| Section 6.5-6.6 | Move to Index File Template (registries) |
| Section 6.7 | Becomes the "Phase Files" table in Index; replace with file reference list |
| Section 6.8 | Becomes the Phase File Template |
| Section 6.9 | Keep inline checkpoints in phase files |
| Section 7-8 | Move templates to Index File |
| Section 9 | Move Traceability Matrix to Index File |
| Section 10 | Move Feedback Template to Index File |
| Section 12 | Replace "one document" with "multi-file bundle" constraint |

### R-05: New Section -- "File Emission Rules"

Add after Section 3 (Artifact Paths):
...
>
> **Naming**: Phase files MUST use the `phase-N-tasklist.md` convention (primary Sprint CLI convention). Do not emit mixed aliases unless explicitly requested.
<!-- Source: Variant B, item 3, lines 67-72 — merged per Change #2 -->
>
> **Phase heading**: MUST be `# Phase N — <Name>` (level 1, em-dash separator, name must not exceed 50 chars).
>
...
<!-- Source: Variant B, lines 118-130 — merged per Change #1 -->

#### Target Directory Layout

The generator output must conform to this directory structure:
...
<!-- Source: Base (original, modified) — wording refined per Change #3 -->

### R-06: New Section -- "Sprint Compatibility Self-Check"

Add before Section 12 (Final Output Constraint):
...
The wrapper prompt needs these updates:

1. **Fix path reference**: `.dev/.releases/` must become `.dev/releases/`
2. **Update output instruction**: Specify the output directory, not a single file
...
| Index with literal filenames | N/A | Yes | Required | N/A |
| Phase heading `# Phase N —` | `## Phase N:` | `# Phase N —` | Required | Compatible |
| Task ID `T<PP>.<TT>` | Yes | Yes | Required | Required |
| Per-task Tier field | Yes | Yes | Via prompt | Required |
| Per-task metadata table | Prose format | Table format | Compatible | Preferred |
| Lean phase files | No (monolith) | Yes | Token savings | Token savings |
| End-of-phase checkpoint | No (cadence only) | Yes | Gate signal | Gate signal |
| Completion protocol in file | No (templates) | No (executor injects) | Correct | Correct |
| Registries in index only | No (in monolith) | Yes | Separated | Less noise |
...
1. Split Section 6 Output Template into Index Template + Phase File Template
2. Replace Section 1/Section 12 single-document constraints
3. Add Section 3.3 File Emission Rules
4. Add Section 11.5 Sprint Compatibility Self-Check
...
2. **Should inline checkpoints (every 5 tasks) remain?**
   Recommendation: Keep them -- they are useful for sc:task-unified even if Sprint does not parse them.
