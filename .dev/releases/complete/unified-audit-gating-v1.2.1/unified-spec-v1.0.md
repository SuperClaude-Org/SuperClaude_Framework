...
### 3.4 The 80% Reimbursement Rate

An 80% reimbursement rate creates natural budget decay even when everything passes:

```
net_cost_per_task = actual_turns - floor(actual_turns × 0.80) + overhead_turns
                  = 8 - floor(6.4) + 2
                  = 8 - 6 + 2
                  = 4 turns per passing task
```

For a 46-task sprint: 46 × 4 = 184 turns of net drain. A 200-turn budget sustains this with 16 turns of margin. This makes infinite-run mathematically impossible while providing generous capacity for legitimate work.
