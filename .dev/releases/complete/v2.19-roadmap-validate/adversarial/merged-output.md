...
<!-- Source: Variant A (roadmap-extract-failure-context.md), "Key Files" — merged per Change #3 -->
### Key Files
...
**Source command/skill references:**
...
<!-- Source: Base (original, modified) — added --verbose investigation note per Change #2 -->
...
**Investigation Note:** The subprocess is invoked with `--verbose` and `--output-format text` flags (see `src/superclaude/cli/pipeline/process.py`). While the observed preamble ("Now I have the full spec...") appears to be LLM conversational text rather than CLI diagnostic output, the `--verbose` flag's interaction with stdout capture should be validated. If `--verbose` contributes to preamble generation through any code path, prompt hardening alone may be insufficient for that class of preamble.
...
- **Before fix:** P(all 8 pass) = 0.9^8 = 43% end-to-end success
...
<!-- Source: Variant A (roadmap-extract-failure-context.md), "Important Mismatch Discovered" — merged per Change #1 -->
## Protocol Drift: Separate Issue

**This section documents a second, independent bug discovered during investigation.**

The programmatic CLI prompt/gate pair is much thinner than the source `sc-roadmap` protocol. The current CLI extract step requests only 3 frontmatter fields:
- `functional_requirements`
- `complexity_score`
- `complexity_class`

The source protocol (`src/superclaude/skills/sc-roadmap-protocol/SKILL.md` + `refs/templates.md`) expects significantly richer extraction output, including:
...
**Impact:** Even after the preamble fix, the CLI pipeline will produce extraction artifacts that are structurally incomplete relative to the source protocol. Downstream steps and integrations that expect the richer field set will fail or produce degraded results.

**Recommendation:** Track as a separate issue. Do not block the preamble fix on this, but do not consider the CLI port complete until protocol parity is achieved.

---

<!-- Source: Variant A (roadmap-extract-failure-context.md), "Constraints for Follow-up" — merged per Change #4 -->
## Evaluation Criteria for Fixes

Any fix should be evaluated against:
- **Exact source protocol parity** — does the fix move toward or away from matching the source `sc-roadmap` protocol?
- **Resumability behavior** — can a partially-completed pipeline resume after the fix is applied?
- **Gate strictness expectations** — does the fix maintain appropriate strictness for later, higher-tier steps (STRICT gates)?
- **Fix location appropriateness** — is the fix best applied in prompt design, subprocess invocation mode, artifact normalization, or a combination?
...
**Additionally tracked:** Protocol drift (10+ missing frontmatter fields) should be addressed as a separate work item to achieve source protocol parity.
