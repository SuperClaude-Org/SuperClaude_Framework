...
Additionally, `_parse_routing_list()` SHALL cross-check `len(returned_tokens)` against the `total_analyzed` frontmatter field. If `len(returned_tokens) > total_analyzed`, a WARNING SHALL be logged (routing more IDs than were analyzed suggests a parse error or duplicate IDs).
