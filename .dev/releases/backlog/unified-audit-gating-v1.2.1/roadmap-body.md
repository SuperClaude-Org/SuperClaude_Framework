# Roadmap: Unified Audit Gating System v1.2.1 [Merged]
...
**Adversarial convergence**: 92% across 3 debate rounds
...
| M2 | State Machine Implementation and Illegal-Transition Test Suite | Implementation + Testing | Critical | High | M1 | `models.py` audit states; transition validator (closed-world); OverrideRecord with compile-time release prohibition; `test_audit_gate_state_machine.py` passing | High -- bypass paths are a direct security/integrity risk |
...
| M4 | Runtime Controls, Override Governance, Override Test Suite, and Reliability Hardening | Implementation + Testing | High | High | M1 (retry values, override model), M2, M3 | Lease/heartbeat/retry logic; `tui.py` guards; `test_audit_gate_overrides.py` passing; fault-injection suite; deadlock-resistance formal argument | High -- incomplete override metadata is a blocking escape path |
...
<!-- Provenance: QA base (V2) structure. M2 enhanced with U-001/U-002. M4 enhanced with U-005. M6 restructured per U-006. -->
...
<!-- Provenance: QA base (V2) dependency graph, unchanged -->
...
<!-- Provenance: QA base (V2) M1, unchanged -->
...
| M1-D2 | Retry/backoff/timeout policy decision document | Document states: maximum retry attempt budget per scope (task, milestone, release); budget semantics (per-entity vs per-run); backoff strategy (constant, linear, or exponential with base values); absolute timeout values in seconds per scope; heartbeat interval and missed-heartbeat threshold; owner name and UTC sign-off date; no [TBD] fields remaining |
...
Implement the normative audit states and transition validator in `models.py`, and build an exhaustive test suite covering every legal and illegal transition defined in spec Sections 4.1 and 4.2. The transition validator uses a **closed-world assumption**: only transitions explicitly listed in the transition table are legal; all other (from_state, to_state) pairs are illegal by default. The OverrideRecord enforces release-scope prohibition at **construction time** (not only at transition validation time), providing defense-in-depth. The test suite must be the authoritative enforcement reference -- not the prose specification -- by the time this milestone closes.

<!-- Provenance: QA base (V2) M2 objective, enhanced with V1 U-001 (closed-world) and U-002 (compile-time override) -->
...
| M2-D8 | OverrideRecord compile-time release prohibition | OverrideRecord constructor/validator rejects scope='release' at instantiation time; test: attempt to create OverrideRecord with scope='release' and all other fields valid; constructor raises ValueError or equivalent; this check is independent of and in addition to the transition validator's release override prohibition (M2-D2) |

<!-- Provenance: QA base (V2) M2-D1 through M2-D7, unchanged. M2-D2 enhanced with closed-world from V1 U-001. M2-D8 added from V1 U-002. -->
...
| Closed-world assumption creates false negatives for legitimate new transitions added later | Low | Medium -- requires transition table update for any new state | Document: any new state MUST be accompanied by explicit transition declarations; add a code comment in the transition table referencing this requirement |

<!-- Provenance: QA base (V2) M2 risks, plus new risk for closed-world from V1 U-001 -->
...
<!-- Provenance: QA base (V2) M3, unchanged -->
...
Additionally, prove that runtime controls withstand adversarial conditions through a dedicated fault-injection suite, and provide a formal argument that no deadlock path exists in the state machine.

<!-- Provenance: QA base (V2) M4 objective, enhanced with V3 U-005 reliability hardening -->
...
| M4-D12 | Fault-injection suite | Transient, system, and timeout fault tests that prove lease/heartbeat/retry controls work under adversarial conditions: (1) inject heartbeat failure mid-run, verify state transitions to failed(timeout); (2) inject lease expiry during evaluation, verify clean failure; (3) inject retry with concurrent heartbeat, verify no state oscillation; (4) inject system fault during state transition, verify atomic state change or clean rollback; pass threshold must be met before M6 can begin |
...
<!-- Provenance: QA base (V2) M4-D1 through M4-D11, unchanged. M4-D12 and M4-D13 added from V3 U-005 (reliability hardening). -->

### Dependencies

- M1-D2 (retry/backoff/timeout values) -- M4-D1, M4-D10, M4-D12 cannot be written without concrete values
...
| Fault-injection tests create non-deterministic timing interactions | Medium | Medium -- flaky fault tests reduce confidence | Use deterministic fault injection (mock failure injection) rather than race-condition-dependent failure; injectable time source for all timeout paths |
| Deadlock-resistance proof incomplete for concurrent gate runs | Low | High -- concurrent runs could create cross-entity deadlocks | Scope the formal argument to single-entity paths; add a documented assumption that cross-entity deadlock prevention is an operational concern addressed by the runtime scheduler |

<!-- Provenance: QA base (V2) M4 risks, plus new risks for V3 U-005 integrations -->
...
<!-- Provenance: QA base (V2) M5, unchanged -->
...
This milestone is structured in two sub-phases with explicit ordering constraints:
- **Sub-Phase A (Shadow + Shadow-to-Soft Gate)**: M6-D1, M6-D5, M6-D6, M6-D7, M6-D9. M6-D9 (Shadow-to-Soft sign-off) must be complete before any Sub-Phase B deliverable can begin.
- **Sub-Phase B (Soft/Full Enforcement + Rollback Drill + Soft-to-Full Gate)**: M6-D2, M6-D3, M6-D4, M6-D8, M6-D10. M6-D8 (Rollback drill) must pass before M6-D10 (Soft-to-Full sign-off) can be approved.

<!-- Provenance: QA base (V2) M6 objective, enhanced with V3 U-006 sub-phase ordering -->
...
| M6-D9 | Shadow-to-Soft promotion gate sign-off | All five promotion criteria from spec Section 7.2 confirmed: M1 (decisions closed), M4 (determinism gate), M5 (evidence gate, per the spec's M5 metric label), M7 (stale-running gate, per spec), M9 (override governance gate, per spec); sign-off by policy owner and program manager; **GATE: Sub-Phase B deliverables cannot begin until M6-D9 is signed off** |
...
| M6-D8 | Rollback drill | Forced rollback from soft to shadow executed; drill validates: rollback completes within defined window; shadow artifact production remains active; no GateResult data lost; completion/release blocking is disabled in the rolled-back phase; drill result documented with pass/fail verdict and drill log; **GATE: M6-D10 cannot be signed off until M6-D8 passes** |
...
<!-- Provenance: QA base (V2) M6 deliverables, restructured into sub-phases per V3 U-006 -->

### Dependencies

- M3 (evaluator and profile tests)
- M4 (runtime controls, override tests, and reliability hardening including fault-injection suite)
...
| Sub-Phase A deliverables delayed, compressing Sub-Phase B timeline | Medium | High -- insufficient time for Soft stabilization windows | Shadow windows should begin as early as possible; M6-D9 sign-off can proceed in parallel with M5 regression gate if M3/M4 are complete |

<!-- Provenance: QA base (V2) M6 risks, plus new risk for sub-phase timing -->
...
| R3 | Release override path discovered or reachable through indirect state manipulation | M2, M4, M6 | Medium | Critical | Enforce prohibition at construction time (M2-D8), at validator (M2-D2), and at persistence layer; add adversarial test (M4-D7) that attempts every known bypass pattern | State-machine owner (TBD per M1) |
...
| R11 | Closed-world assumption creates friction when adding new states | M2 | Low | Medium | Document requirement: any new state MUST be accompanied by explicit transition declarations; code comment in transition table | State-machine owner |
| R12 | Fault-injection tests produce non-deterministic results due to timing | M4 | Medium | Medium | Use deterministic mock-based fault injection; injectable time source for all timeout/heartbeat paths | Reliability owner |
| R13 | Deadlock-resistance argument incomplete for concurrent multi-entity scenarios | M4 | Low | High | Scope formal argument to single-entity paths; document cross-entity assumption; add operational runbook for concurrent scenarios | Reliability owner |

<!-- Provenance: QA base (V2) R1-R10, plus R11 (V1 U-001), R12-R13 (V3 U-005) -->
...
|----------|--------|-------------------------|-----------|
| Blocker resolution as standalone M1 | Standalone prerequisite milestone | Resolve blockers in parallel with early implementation (V1 M6); resolve inline with contracts (V1 M1 provisional values) | Non-deterministic inputs produce non-deterministic tests; any test written before M1 closes is untrustworthy and likely requires rewrite |
| Testing milestones interleaved with implementation | Interleaved per milestone | Single validation sprint at the end; separate test-only milestones | The feature is a gating mechanism; deferring testing means the gate is unvalidated while it is being relied upon in shadow mode |
...
| Rollback drill as non-waivable gate for Soft-to-Full | Non-waivable | Optional/best-effort drill | Spec Section 7.2 explicitly requires rollback drill passage; any waiver would violate the rollout contract |
| Sprint CLI regression gate (M5) as standalone milestone | Standalone milestone | Folded into M2 or M4 (V1 approach); omitted (V3 approach) | `models.py` and `tui.py` are shared infrastructure; regression scope is broad enough to warrant dedicated tracking |
| Transition validator uses closed-world assumption | Closed-world (only listed transitions legal) | Open-world (only listed transitions illegal, V1 alternative considered) | Closed-world eliminates bypass categories by default; any new state requires explicit transition declaration |
| Release override prohibition at construction time | Compile-time enforcement (OverrideRecord constructor rejects release scope) | Runtime check only (transition validator) | Catches invalid overrides at creation, not at transition time; defense in depth with two independent enforcement layers |
| Adversarial release override test with fully valid record | Adversarial test with valid record at forbidden scope | Test only with absent record | The dangerous failure mode is a valid record being incorrectly accepted at release scope; testing only absent records does not validate the prohibition |
| Rollout milestone structured with sub-phase ordering | Single M6 with Sub-Phase A (Shadow + gate) and Sub-Phase B (Soft/Full + drill + gate) | Three separate milestones (V3 approach); single flat milestone (V2 original) | Sub-phases provide V3's ordering guarantees without consuming extra milestones; prevents Soft enforcement before Shadow-to-Soft is approved |
| Fault-injection suite as explicit M4 deliverable | Explicit deliverable within M4 | Implicit testing within runtime implementation (V2 original); dedicated reliability milestone (V3 approach) | Explicit deliverable ensures reliability is proven before rollout; within M4 rather than separate milestone to avoid milestone proliferation |

<!-- Provenance: QA base (V2) decisions, plus decisions for V1 U-001, V1 U-002, V3 U-005, V3 U-006 integrations -->
...
| SC-4 | Closed-world enforcement: synthetic state not in transition table returns allowed=False | M2 | Yes -- one explicit test asserting unknown state pair is rejected |
| SC-5 | OverrideRecord with scope='release' rejected at construction time (ValueError or equivalent) | M2 | Yes -- explicit test in M2-D8 |
| SC-6 | Stuck-state recovery: retry budget exhaustion produces audit_*_failed with no completion path; rollback path from audit_*_failed re-enters ready_for_audit_* correctly | M2 | Yes -- test assertions on return values of transition validator after retry budget exceeded |
| SC-7 | Profile determinism: identical inputs produce identical GateResult (status, score, failure_class, check outcomes) across 3 replays for each of 3 profiles | M3 | Yes -- 9 determinism assertions in CI; all pass |
| SC-8 | Evidence completeness: every failed check contains a file path or file:line reference; missing evidence makes the check non-completable | M3 | Yes -- test assertions on checks[] in GateResult; zero failed checks with empty evidence_refs |
| SC-9 | Release override is unconditionally blocked even when a fully valid OverrideRecord is supplied | M4 | Yes -- test asserts allowed=False; reason string contains "release override forbidden" |
| SC-10 | Incomplete override metadata (any single required field absent) blocks completion with an error naming the missing field | M4 | Yes -- 11+ individual field-absence tests all pass |
| SC-11 | Fault-injection suite passes: heartbeat failure, lease expiry, concurrent retry, and system fault scenarios all result in clean state transitions | M4 | Yes -- 4 fault-injection test scenarios pass |
| SC-12 | Deadlock-resistance: 10K random state sequences all terminate within bounded steps | M4 | Yes -- property-based test with termination assertion |
| SC-13 | Zero regressions in `tests/sprint/` after M2 and M4 changes land on the integration branch | M5 | Yes -- CI failing test count is 0 |
| SC-14 | All pre-existing PhaseStatus and SprintOutcome enum values retain identical is_terminal, is_success, is_failure semantics | M5 | Yes -- explicit backward-compatibility assertions pass |
| SC-15 | Two consecutive shadow windows complete with evidence completeness rate = 100% and determinism rate at or above M1-D1 floor | M6 | Yes -- KPI report filed and approved by policy owner |
| SC-16 | Rollback drill executed and documented with pass verdict before Soft-to-Full promotion sign-off | M6 | Yes -- drill log attached to M6-D10 sign-off document |
| SC-17 | All five rollback trigger tests pass in `test_audit_gate_rollout.py` | M6 | Yes -- 5 trigger tests in CI; all pass |
| SC-18 | `--strictness` alias rejected with clear error in full enforcement mode; accepted with deprecation warning in shadow/soft | M3, M6 | Yes -- two explicit tests, both pass |
| SC-19 | Soft-to-Full promotion sign-off includes: policy owner, reliability owner, program manager signatures; no outstanding NO-GO criteria from spec Section 12.2 | M6 | Yes -- sign-off document contains three named approvers; checklist Section 10 status is PASS |
| SC-20 | Sub-Phase A (Shadow-to-Soft) signed off before any Sub-Phase B deliverable begins | M6 | Yes -- M6-D9 sign-off timestamp precedes M6-D2/D3/D4/D8 start timestamps |

<!-- Provenance: QA base (V2) SC-1 through SC-15, plus SC-4/SC-5 (V1 U-001/U-002), SC-11/SC-12 (V3 U-005), SC-20 (V3 U-006) -->
