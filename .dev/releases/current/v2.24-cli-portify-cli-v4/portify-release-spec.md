...
| `src/superclaude/cli/cli_portify/models.py` | PortifyConfig, PortifyStatus, PortifyOutcome, PortifyStepResult, PortifyResult, PortifyMonitorState | pipeline.models |
| `src/superclaude/cli/cli_portify/gates.py` | 7 gate definitions + 7 semantic check functions | pipeline.models, models.py |
| `src/superclaude/cli/cli_portify/prompts.py` | 5 prompt builders (analyze, pipeline, synthesize, brainstorm, panel) | models.py |
| `src/superclaude/cli/cli_portify/config.py` | `load_cli_portify_config()`, name derivation, path resolution | models.py |
| `src/superclaude/cli/cli_portify/inventory.py` | `run_validate_config()`, `run_discover_components()` pure-programmatic steps | models.py |
| `src/superclaude/cli/cli_portify/monitor.py` | OutputMonitor with domain signals (phase, persona, convergence iteration, placeholders) | models.py |
| `src/superclaude/cli/cli_portify/process.py` | PortifyProcess extending pipeline.ClaudeProcess; passes `--add-dir` for work_dir and workflow_path via `extra_args` so subprocess can read `@path` referenced files [GAP-002] | models.py, config.py, pipeline.process |
| `src/superclaude/cli/cli_portify/executor.py` | `execute_cli_portify()`, convergence loop, programmatic step routing. Note: high fan-in (imports all other modules) is inherent to the executor pattern, consistent with sprint/executor.py [F-001] | all above modules |
| `src/superclaude/cli/cli_portify/tui.py` | Rich live dashboard with gate state machine | models.py |
| `src/superclaude/cli/cli_portify/logging_.py` | Dual JSONL + Markdown execution logging | models.py |
| `src/superclaude/cli/cli_portify/diagnostics.py` | DiagnosticCollector, FailureClassifier, ReportGenerator | models.py |
| `src/superclaude/cli/cli_portify/commands.py` | Click CLI group (`cli-portify`) and `run` subcommand | config.py, executor.py |
...
cli_portify/models.py ◄──────────────────────────────────────┘│││
    |                                                         │││
    ├── cli_portify/gates.py ◄────────────────────────────────┘││
    ├── cli_portify/prompts.py                                 ││
    ├── cli_portify/config.py                                  ││
    ├── cli_portify/inventory.py                               ││
    ├── cli_portify/monitor.py                                 ││
    ├── cli_portify/process.py ◄───────────────────────────────┘│
    ├── cli_portify/tui.py                                      │
    ├── cli_portify/logging_.py                                 │
    └── cli_portify/diagnostics.py                              │
                                                                │
cli_portify/executor.py ◄── (imports all above) ───────────────┘
    |
cli_portify/commands.py ◄── config.py, executor.py
    |
cli_portify/__init__.py ◄── commands.py
    |
...
2. gates.py           -- Imports from models; 7 semantic checks + 7 gate definitions
   prompts.py         -- [parallel with gates.py] Imports from models; 5 prompt builders
   config.py          -- [parallel with gates.py] Imports from models; load_cli_portify_config()
3. inventory.py       -- Imports from models; 2 pure-programmatic step implementations
4. monitor.py         -- Imports from models; OutputMonitor with domain signals
   process.py         -- [parallel with monitor.py] Imports from models, config; extends pipeline.process
5. tui.py             -- Imports from models; Rich live dashboard
   logging_.py        -- [parallel with tui.py] Imports from models; dual JSONL + Markdown
   diagnostics.py     -- [parallel with tui.py] Imports from models; failure classification
6. executor.py        -- Imports from all above; main execution loop + convergence handler
7. commands.py        -- Imports from config, executor; Click CLI group
8. __init__.py        -- Re-exports commands group
9. main.py (patch)    -- Add import + app.add_command()
```
