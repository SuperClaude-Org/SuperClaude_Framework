...
| D1.4 | `pipeline/process.py` — `ClaudeProcess` extracted from sprint | `output_format` parameter added (default: `stream-json` for sprint backward-compat; roadmap specifies `text` for gate-compatible output); sprint re-exports from this module |
...
Migrate `sprint/` to consume the `pipeline/` module for `ClaudeProcess`, `PipelineConfig`, and related types. Sprint's external CLI API must remain identical (NFR-001). All sprint test files passing at extraction start must continue to pass without modification during pipeline/ migration (NFR-002). Note: v2.07 modified 9 of the original 14 test files; the baseline is the post-v2.07 state.
...
| D2.4 | Sprint regression validation | `uv run pytest tests/sprint/` exits 0 with all sprint test files passing at extraction start; no sprint test modifications during pipeline/ migration; stall watchdog behavior (--stall-timeout, --stall-action) exercised by existing sprint tests continues to pass post-migration |
...
| release_dir alias chain resolution | Low | Medium | Property alias must resolve transitively for all SprintConfig computed properties that depend on release_dir (debug_log_path, results_dir, etc.); existing sprint tests exercise these paths |
...
| D5.3 | Sprint regression verification | `uv run pytest tests/sprint/` exits 0; all sprint test files present at extraction start pass without modification during migration (AC-06) |
