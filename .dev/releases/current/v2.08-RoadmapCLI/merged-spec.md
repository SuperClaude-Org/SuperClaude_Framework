...
<!-- Source: Variant B (tasklist-strategies), Strategy 1 rationale — merged per Change #3 -->

**Design principle — no circular self-validation**: The conductor enforces a strict trust boundary. Gate validation (`gate_passed()`) is pure Python — it never invokes Claude to evaluate Claude's output. This eliminates the circular self-validation failure mode where the same model that produced an artifact judges its own quality, which produces false confidence regardless of how sophisticated the validation criteria are. The insight that "the same model that produces output cannot reliably validate its own intermediate state" (from adversarial analysis of tasklist pipeline strategies) is the architectural justification for externalizing all gate logic into deterministic Python.
...
<!-- Source: Variant B (tasklist-strategies), scope discipline — merged per Change #4 -->
...
- **No self-validation**: Gate checks are pure Python. The conductor never invokes Claude to judge Claude's output (see §1 design principle).
- **Deferral tracking**: Features considered and deferred are recorded in §11 with rationale, preserving decision provenance for future versions.
...
src/superclaude/cli/
  pipeline/                  ← NEW: shared base, both sprint and roadmap extend it
    __init__.py
    process.py               ← ClaudeProcess (extracted from sprint/process.py; output_format parameterized: sprint=stream-json, roadmap=text)
...
`SemanticCheck`:
<!-- Source: Variant B, Strategies 3+4+5 — merged per Change #2 -->

```python
@dataclass
class SemanticCheck:
    """Pure Python check applied to file content. No LLM invocation."""
    name: str                           # e.g. "no_heading_gaps"
    check_fn: Callable[[str], bool]     # receives file content, returns pass/fail
    failure_message: str                # human-readable on failure
```
...
def gate_passed(output_file: Path, criteria: GateCriteria) -> tuple[bool, str | None]:
    ...
    Validation is tier-proportional (see §3.3 Gate Enforcement Tiers):
    - EXEMPT: always passes
    - LIGHT: file exists + non-empty
    - STANDARD: file exists + non-empty + min lines + frontmatter fields
    - STRICT: all STANDARD checks + semantic checks (if defined)
    ...
    - "Semantic check '{name}' failed: {failure_message}"
    """
```

Note: `gate_passed()` is pure Python — no subprocess, no Claude invocation.

### 3.3 Gate Enforcement Tiers
<!-- Source: Variant B (tasklist-strategies), Strategy 4 — merged per Change #1 -->

Gate validation intensity is tier-proportional. Each step's `GateCriteria` includes an `enforcement_tier` that controls which checks are applied:

| Tier | File Exists | Non-Empty | Min Lines | Frontmatter Fields | Semantic Checks |
|------|:-:|:-:|:-:|:-:|:-:|
| **STRICT** | ✓ | ✓ | ✓ | ✓ | ✓ |
| **STANDARD** | ✓ | ✓ | ✓ | ✓ | — |
| **LIGHT** | ✓ | ✓ | — | — | — |
...
**Step-to-tier mapping** (v1 defaults):

| Step | Tier | Rationale |
|------|------|-----------|
| `extract` | STANDARD | Bounded single-concern task; frontmatter sufficient |
| `generate-{agent}` | STRICT | Most expensive step; highest fabrication risk |
| `diff` | STANDARD | Structured output with countable diff points |
| `debate` | STRICT | Complex multi-round output; convergence score critical |
| `score` | STANDARD | Selection decision with scoring breakdown |
| `merge` | STRICT | Final synthesis; structural integrity critical |
| `test-strategy` | STANDARD | Bounded single-concern task |
...
**Semantic checks for STRICT-tier steps** (v1):
<!-- Source: Variant B, Strategy 5 — merged per Change #5 -->

The `merge` step includes these additional `SemanticCheck` instances:
- `no_heading_gaps`: No heading level gaps (e.g., H2 → H4 without H3)
- `cross_refs_resolve`: All "See §X" and "Section X.Y" references resolve to existing headings
- `no_duplicate_headings`: No duplicate H2 or H3 heading text

The `generate-{agent}` steps include:
- `frontmatter_values_non_empty`: All required frontmatter fields have non-empty values
- `has_actionable_content`: At least one section contains numbered or bulleted items (not prose-only)

The `debate` step includes:
- `convergence_score_valid`: `convergence_score` frontmatter value parses as float in [0.0, 1.0]
...
## 11. Open Questions & Deferred Features
...
<!-- Source: Variant B (tasklist-strategies), v1.1 deferral table — merged per Change #6 -->

| Feature | Considered In | Deferral Reason |
|---------|--------------|-----------------|
| Multi-spec consolidation (`--specs`) | §2 Out of Scope | Requires adversarial merge across >2 variants; validate 2-agent pipeline first |
| `--blind` adversarial mode | §2 Out of Scope | Adds complexity without proven value in v1 |
| >2 agents | §2 Out of Scope | v1 validates 2-agent pipeline before scaling |
| TUI / rich progress display | §2 Out of Scope | stdout logging sufficient for v1; sprint TUI available for reference |
| Tier-proportional semantic gates (full) | §3.3, this merge | Framework designed; full semantic check library deferred to v1.1 |
| Content quality heuristics for input | Adversarial analysis | Duplicates parser logic; rejected per tasklist debate outcomes |
| Per-step halt-on-failure with LLM validation | Adversarial analysis | Circular self-validation; external Python gates used instead |
...
2. Move `ClaudeProcess` from `sprint/process.py` to `pipeline/process.py`; re-export from `sprint/process.py`. Note: sprint/process.py contains `debug_log()` calls tied to sprint's debug logger. When extracting, replace with stdlib logging or remove (NFR-007 prohibits pipeline/ from importing sprint modules).
...
| `debug: bool` | **✓** | — | Both pipelines support debug logging. PipelineConfig provides the flag; logging infrastructure is consumer-specific (sprint owns `debug_logger.py`, roadmap provides its own). |
...
**Sprint test stability guarantee**: All sprint test files passing at extraction start are not modified during pipeline/ migration. Sprint's `process.py` re-exports `ClaudeProcess` from `pipeline.process`, so `from superclaude.cli.sprint.process import ClaudeProcess` continues to resolve. If any sprint test imports break, the migration has a bug — this is the canary. Note: v2.07 modified 9 of the original 14 sprint test files; the regression baseline is the post-v2.07 state, not the original test suite.
